...
왼쪽 **시장 지도**에서 KOSPI·KOSDAQ·NASDAQ을 선택합니다. 업종 필터와 종목 선택 상세, 전체 종목 목록을 지원합니다.
`시장데이터 업데이트.cmd` 또는 기존 `collect-market.ps1` 실행 시 세 시장 지도도 갱신됩니다.

- 공급자 시가총액 상위 최대 100종목씩 표시합니다. 한국 목록의 ETF는 제외하고 다음 페이지를 확인해 종목을 보충합니다.
- 한국: 네이버 공개 시가총액 목록·업종 분류. 원 단위 시가총액, 기본 가격·등락률을 사용하며 NXT 시간외 필드는 사용하지 않습니다.
- 미국: Nasdaq 공개 screener 다운로드의 나스닥 상장 종목. **NASDAQ-100 지수 구성종목이 아닙니다.**
- 면적은 공급자 시가총액, 색상은 이전 종가 대비 등락률(상승 초록·하락 빨강·보합 회색)입니다.
- 업종 헤더 등락률은 표시 종목의 현재 시가총액 가중 평균이며 공식 업종·시장 지수 수익률이 아닙니다.
- 미국 복수 주식종류는 별도 종목으로 유지하므로 공급자 기업 시가총액이 중복될 수 있습니다. 미국 개별 거래시각은 제공되지 않아 수집시각만 표시합니다.
- 업종은 제공처 분류이며 임의로 AI 테마를 생성하지 않습니다. 한국 업종 분류는 30일 캐시로 반복 요청을 줄이고, 갱신 실패 시 이전 분류를 유지합니다.
- 한 시장 수집이 실패하면 그 시장의 이전 지도와 원래 수집시각을 유지합니다. 없는 데이터를 생성하지 않습니다.

저장: `market-data/heatmap/latest.json`, 날짜별 `runs/`, 원응답 `raw/`, 분류 캐시 `industries.json`.
사이트는 숫자 전용 `data/heatmap.js`를 읽습니다. 기존 리서치나 AI Confidence Score를 수정하지 않습니다.
작은 화면에서는 지도를 가로로 이동하거나 종목 목록을 펼쳐 확인할 수 있습니다.
